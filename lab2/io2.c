...
{
    int ret=0;
    printk(KERN_INFO "vramdisk : open \n");
...
{
    printk(KERN_INFO "vramdisk : closed \n");
}
...
static int rb_transfer(struct request *req)
{
    ...
    {
        printk(KERN_ERR "vramdisk: bio info doesn't match with the request info");
        ret = -EIO;
    }
    return ret;
}
...
{
    mydisk_init();
    c = register_blkdev(c, "vramdisk");// major no. allocation
